...
describe Stanford::SaltDocument do
  ...
  describe "it should itialize correctly" do
    ...
    it "should raise an error if it have a pid" do
      expect { Stanford::SaltDocument.new("") }.to raise_error(ArgumentError, "Must have a PID for the salt document")
      expect { Stanford::SaltDocument.new() }.to raise_error(ArgumentError, /wrong number of arguments/)
      expect { Stanford::SaltDocument.new([]) }.to raise_error(ArgumentError, "Must have a PID for the salt document")
    end 
     ...
     it "should assign the asset_repo correctly if passed in on new" do
       ...
       expect(@salt_doc.asset_repo).to eq("http://fakesite.com")
     end
     
   
     it "should have the instance variables properyl set up by default" do
       expect(Stanford::Repository).to receive(:new).and_return(@mock_salt_doc_repo)
       @salt_doc = Stanford::SaltDocument.new("druid:123")
       expect(@salt_doc.datastreams).to eq({"extracted_entities" => "<xml/>", "zotero" => "<xml/>"})
       expect(@salt_doc.solr_document).to eq({"id" => ["druid:123"]})
       expect(@salt_doc.repository).to eq(@mock_salt_doc_repo)
       expect(@salt_doc.pid).to eq("druid:123")
     end
     ...
     it "should be allow for defaults to be overridden" do
       ...
       expect(@salt_doc.datastreams).to eq({"foo" => "<xml/>", "bar" => "<xml/>"})
       expect(@salt_doc.datastreams).not_to eq({"extracted_entities" => "<xml/>", "zotero" => "<xml/>"})
       expect(@salt_doc.solr_document).to eq({"id" => ["druid:456"]})
       expect(@salt_doc.repository).to eq(@mock_salt_doc_repo2)
       expect(@salt_doc.repository).not_to eq(@mock_salt_doc_repo)
       expect(@salt_doc.pid).to eq("druid:456")
  ...
  describe "#to_solr" do 
    ...
    it "should return a hash and store it in @solr_document" do
      ...
      expect(solr_doc["id"]).to eq(["druid:123"])
      expect(solr_doc["access_display"]).to eql(["Private"])
    ...
    it "should run the _to_solr methods for all datastreams in the @datastreams" do
      ...
      expect(salt_doc).to receive(:extracted_entities_to_solr).once
      expect(salt_doc).to receive(:zotero_to_solr).once
      salt_doc.to_solr
    end
    ...
    it "should raise not run an default methods if the datastream does not have a matching method when using #to_solr" do
      ...
      expect(salt_doc).to receive(:extracted_entities_to_solr).never
      expect(salt_doc).to receive(:zotero_to_solr).never
  ...
  describe "fulltext_to_solr" do
    ...
    it  "should get the full text index from the alto files" do 
       ...
       expect(@salt_doc.solr_document["text"]).to eq(["this is alto"])
    end  
  ...
  describe "#get_alto" do
    ...
    it "should get the alto file for a page" do
      ...
      # @mock_salt_doc_asset_repo.stub(:get_json => { "pages" => [ "a page" ]})
       ...
       @salt_doc = Stanford::SaltDocument.new("druid:456", {:asset_repo=> @mock_salt_doc_asset_repo })  
       expect(@salt_doc.get_alto("1")).to eq(xml)
    end
    
    it "should return nil if there's an exception raised" do
       @mock_salt_doc_asset_repo = double("Stanford::AssetRepository")
       allow(@mock_salt_doc_asset_repo).to receive(:get_page_xml).and_raise(StandardError.new("meatball problems."))
      
       @salt_doc = Stanford::SaltDocument.new("druid:456", {:asset_repo=> @mock_salt_doc_asset_repo })  
       expect(@salt_doc.get_alto("1")).to be_nil
    end
  ...
  describe "#*_to_solr" do
    before(:each) do 
       @mock_salt_doc_repo = double("Stanford::Repository")
       allow(@mock_salt_doc_repo).to receive(:get_datastream).with("druid:123", "extracted_entities").and_return(IO.read(fixture("extracted_entities_ds.xml").path))
       allow(@mock_salt_doc_repo).to receive(:get_datastream).with("druid:123", "zotero").and_return(IO.read(fixture("zotero_ds.xml").path))
    ...
    it "#extracted_entities_to_solr should update the @solr_document hash with proper values mapped from the extracted_entities datastream" do
      ...
      @salt_doc.extracted_entities_to_solr.each { |k,v| 
        expect(v).to match_array(expected[k])
      }  
    end
  ...
  context "public / private flags" do
    context "public records" do
      before(:each) do 
        ...
        allow(@mock_salt_doc_repo1).to receive(:get_datastream).with("druid:public_test", "extracted_entities").and_return(IO.read(fixture("extracted_entities2_ds.xml").path))
        allow(@mock_salt_doc_repo1).to receive(:get_datastream).with("druid:public_test", "zotero").and_return(IO.read(fixture("zotero_ds.xml").path))
        expect(Stanford::Repository).to receive(:new).and_return(@mock_salt_doc_repo1)
      ...
      it "marks records private by default" do
        ...
        expect(defaults["public_b"]).to eql(['false'])
        expect(defaults["access_display"]).to eql(["Private"])
        expect(defaults["access_facet"]).to eql(["Private"])
      end
      it "processes dc:subject=PUBLIC tags" do
        @public_salt_doc.zotero_to_solr
        expect(@public_salt_doc.solr_document["public_b"]).to eql(['true'])
        expect(@public_salt_doc.solr_document["access_display"]).to eql(["Public"])
        expect(@public_salt_doc.solr_document["access_facet"]).to eql(["Public"])
      end
    end
    context "private records" do
      before(:each) do 
         @mock_salt_doc_repo2 = double("Stanford::Repository")
         allow(@mock_salt_doc_repo2).to receive(:get_datastream).with("druid:private_test", "extracted_entities").and_return(IO.read(fixture("extracted_entities2_ds.xml").path))
         allow(@mock_salt_doc_repo2).to receive(:get_datastream).with("druid:private_test", "zotero").and_return(IO.read(fixture("zotero2_ds.xml").path))
         expect(Stanford::Repository).to receive(:new).and_return(@mock_salt_doc_repo2)
      ...
      it "processes dc:subject=PRIVATE tags" do
        ...
        expect(@private_salt_doc.solr_document["public_b"]).to eql(['false'])
        expect(@private_salt_doc.solr_document["access_display"]).to eql(["Private"])
        expect(@private_salt_doc.solr_document["access_facet"]).to eql(["Private"])
      end
    end
  end
  ...
  describe "#format_coverage" do
    before(:each) do 
       @mock_salt_doc_repo = double("Stanford::Repository")
       allow(@mock_salt_doc_repo).to receive(:get_datastream).with("druid:123", "extracted_entities").and_return(IO.read(fixture("extracted_entities2_ds.xml").path))
       allow(@mock_salt_doc_repo).to receive(:get_datastream).with("druid:123", "zotero").and_return(IO.read(fixture("zotero2_ds.xml").path))
